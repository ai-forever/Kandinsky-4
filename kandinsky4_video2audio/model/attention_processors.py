...
class VideoAttnProcessor2_0(nn.Module):
    ...
    def __call__(
        ...
        pos_emb = get_posemb(torch.arange(key.shape[2], device=key.device), key.shape[-1])
        key += pos_emb
        value += pos_emb
        ...
        hidden_states_video = hidden_states_video.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim).to(query.dtype)

        if input_ndim == 4:
            hidden_states_video = hidden_states_video.transpose(-1, -2).reshape(batch_size, channel, height, width)
...
class PromptAttnProcessor2_0(nn.Module):
    ...
    def __call__(
        ...
        return self.processor(attn, hidden_states, encoder_hidden_states, attention_mask, temb)